...
❌ **Title mơ hồ: "Lỗi trang checkout", "Bug login", "Không hoạt động"**
→ ✅ Dùng format `[Module] Action → Wrong Result`: "[Checkout] Click Place Order with expired card → Error 500"
→ 💡 Dev nhận 20 bugs/ngày. Title mơ hồ = dev không biết priority, không biết bắt đầu từ đâu

❌ **Đổ lỗi cho developer trong bug report: "Dev code sai function X"**
→ ✅ Report facts: "Total hiển thị 0đ khi cart có 3 items. Expected: 1,500,000đ"
→ 💡 Blame tạo defensive reaction → dev mất thời gian bảo vệ thay vì fix. Facts tạo collaboration → fix nhanh hơn

❌ **Không đính kèm evidence (screenshot, video, console log)**
→ ✅ Mỗi bug PHẢI có ít nhất 1 screenshot hoặc video + console log nếu có error
→ 💡 "Mắt thấy tai nghe" — screenshot là bằng chứng không thể chối cãi. Không evidence = dev phải tự reproduce, mất thời gian gấp đôi

❌ **Report bug mà không search xem đã có ai report chưa**
→ ✅ Trước khi tạo bug mới, search Jira bằng keyword (module name, error message)
→ 💡 Duplicate bugs gây noise, waste dev time đọc + close, và làm giảm uy tín của QA. Bug Rejection Rate > 15% = red flag

❌ **Không ghi reproduction rate — "lỗi này hên xui"**
→ ✅ Ghi rõ: "Reproduce 100% (5/5)" hoặc "~30% (3/10), thường xảy ra khi load nặng"
→ 💡 Dev cần biết bug stable hay intermittent để chọn cách debug phù hợp
